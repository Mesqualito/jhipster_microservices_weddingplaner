...
public class WeddingHostResourceIntTest {
    ...
    @Autowired
    private WeddingHostSearchRepository weddingHostSearchRepository;
    ...
    public void initTest() {
        weddingHostSearchRepository.deleteAll();
    ...
    public void createWeddingHost() throws Exception {
        ...
        // Validate the WeddingHost in Elasticsearch
        WeddingHost weddingHostEs = weddingHostSearchRepository.findOne(testWeddingHost.getId());
        assertThat(weddingHostEs).isEqualToIgnoringGivenFields(testWeddingHost);
    }
    ...
    public void deleteWeddingHost() throws Exception {
        ...
        boolean weddingHostExistsInEs = weddingHostSearchRepository.exists(weddingHost.getId());
        assertThat(weddingHostExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchWeddingHost() throws Exception {
        // Initialize the database
        weddingHostRepository.saveAndFlush(weddingHost);
        weddingHostSearchRepository.save(weddingHost);

        // Search the weddingHost
        restWeddingHostMockMvc.perform(get("/api/_search/wedding-hosts?query=id:" + weddingHost.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(weddingHost.getId().intValue())))
            .andExpect(jsonPath("$.[*].firstName").value(hasItem(DEFAULT_FIRST_NAME.toString())))
