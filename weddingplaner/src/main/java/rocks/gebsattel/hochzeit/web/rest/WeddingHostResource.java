...
public class WeddingHostResource {
    ...
    /**
     * SEARCH  /_search/wedding-hosts?query=:query : search for the weddingHost corresponding
     * to the query.
     *
     * @param query the query of the weddingHost search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/wedding-hosts")
    @Timed
    public ResponseEntity<List<WeddingHostDTO>> searchWeddingHosts(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of WeddingHosts for query {}", query);
        Page<WeddingHostDTO> page = weddingHostService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/wedding-hosts");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
