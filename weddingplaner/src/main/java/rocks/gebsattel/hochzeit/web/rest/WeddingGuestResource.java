...
public class WeddingGuestResource {
    ...
    /**
     * SEARCH  /_search/wedding-guests?query=:query : search for the weddingGuest corresponding
     * to the query.
     *
     * @param query the query of the weddingGuest search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/wedding-guests")
    @Timed
    public ResponseEntity<List<WeddingGuestDTO>> searchWeddingGuests(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of WeddingGuests for query {}", query);
        Page<WeddingGuestDTO> page = weddingGuestService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/wedding-guests");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
